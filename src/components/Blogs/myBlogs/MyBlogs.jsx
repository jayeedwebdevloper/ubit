...
const MyBlogs = () => {
    ...
    const { data: blogs = [], refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await fetch(`https://ubit-back-end.vercel.app/blogs/${user?.email}`);
    ...
    const handleUpdate = (id, e, i) => {
        ...
        fetch(`https://ubit-back-end.vercel.app/blog/${id}`, {
            method: "PATCH",
            headers: {
            ...
            .then(data => {
                if (data.modifiedCount > 0) {
                    toast.success("Your Blog Successfully Updated");
    ...
    return (
        <div className='container mx-auto px-4'>
            <h1 className='text-xl font-semibold text-gray-700'>My Blogs</h1>
                                    ...
                                    <tr key={blog._id}>
                                        ...
                                        <td>
                                            <div className="flex gap-3">
                                                ...
                                                <button className='btn btn-sm btn-ghost'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-trash" viewBox="0 0 16 16">
                                                ...
                                                <div className="modal-box">
                                                    ...
                                                    <form onSubmit={(e) => handleUpdate(blog._id, e, i)} className='form-control'>
                                                        <div className="py-2">
                                                            ...
                                                            <input type="text" className='input input-bordered rounded-md w-full' name="title" id="thisName" defaultValue={blog.title} />
                                                        </div>
                                                        <div className="py-2">
                                                            <label htmlFor="regularPriceThis" className='label font-semibold text-md'>Description</label>
                                                            <textarea className='textarea textarea-bordered rounded-md w-full' name="description" id="regularPriceThis" defaultValue={blog.description} />
                                                        </div>
